...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (email) {
      const customerHtml = `
        <div style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto;color:#26466c;">
          <h2 style="color:#0b3c78;">${esc(t.greeting)} ${esc(name)},</h2>
          <p>${esc(t.body1)}</p>
          <p style="font-size:20px;font-weight:bold;background:#f4f7fb;padding:12px 16px;border-radius:8px;display:inline-block;">
            ${esc(t.pnrLabel)}: <span style="color:#2579e0;">${esc(pnr || '—')}</span>
          </p>
          <table style="margin-top:16px;font-size:14px;">
            <tr><td style="padding:4px 8px 4px 0;color:#68809d;">${esc(t.tour)}</td><td>${esc(tourName)}</td></tr>
            <tr><td style="padding:4px 8px 4px 0;color:#68809d;">${esc(t.date)}</td><td>${esc(date)}</td></tr>
            <tr><td style="padding:4px 8px 4px 0;color:#68809d;">${esc(t.people)}</td><td>${esc(String(people))}</td></tr>
            <tr><td style="padding:4px 8px 4px 0;color:#68809d;">${esc(t.phone)}</td><td>${esc(phone)}</td></tr>
            ${note ? `<tr><td style="padding:4px 8px 4px 0;color:#68809d;">${esc(t.note)}</td><td>${esc(note)}</td></tr>` : ''}
          </table>
          <p style="margin-top:20px;font-size:13px;color:#68809d;">${esc(t.footer)}</p>
        </div>`;
    ...
    const businessHtml = `
      <div style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto;color:#26466c;">
        <h2 style="color:#0b3c78;">New website reservation — ${esc(pnr || '—')}</h2>
        <table style="font-size:14px;">
          <tr><td style="padding:4px 8px 4px 0;color:#68809d;">Name</td><td>${esc(name)}</td></tr>
          <tr><td style="padding:4px 8px 4px 0;color:#68809d;">Tour</td><td>${esc(tourName)}</td></tr>
          <tr><td style="padding:4px 8px 4px 0;color:#68809d;">Date</td><td>${esc(date)}</td></tr>
          <tr><td style="padding:4px 8px 4px 0;color:#68809d;">People</td><td>${esc(String(people))}</td></tr>
          <tr><td style="padding:4px 8px 4px 0;color:#68809d;">Phone</td><td>${esc(phone)}</td></tr>
          <tr><td style="padding:4px 8px 4px 0;color:#68809d;">Email</td><td>${esc(email || '—')}</td></tr>
          <tr><td style="padding:4px 8px 4px 0;color:#68809d;">Language</td><td>${esc(lang)}</td></tr>
          ${note ? `<tr><td style="padding:4px 8px 4px 0;color:#68809d;">Note</td><td>${esc(note)}</td></tr>` : ''}
        </table>
      </div>`;
