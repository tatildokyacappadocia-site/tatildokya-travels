...
// Storm/CB scan radius capped at the same 10 km operational ring, per pilot
// judgment (Vahit SERTTAŞ) — a wider 20/30 km scan was flagging cells too
// far away to realistically affect the Göreme launch window, producing
// over-cautious cancellation risk on days that were actually flyable.
const STORM_RADII_KM: number[] = [];
...
// Primary wind criterion, per SHT-OPS-B Madde 6: for passenger flights,
// balloon operations are not conducted when SUSTAINED GROUND WIND ("yer
// rüzgârı") at the moment/place of takeoff exceeds 10 kt (8 kt for
// solo-student flights, not applicable here). This is the actual binding
// regulatory threshold — it is defined on sustained wind, not gust.
function windProbability(sustainedWindKt: number) {
  if (!Number.isFinite(sustainedWindKt)) return 100;

  if (sustainedWindKt > 10) return 0;   // exceeds SHT-OPS-B passenger-flight limit
  if (sustainedWindKt >= 8) return 40;  // close to the limit
  if (sustainedWindKt >= 6) return 70;
  if (sustainedWindKt >= 4) return 90;
  return 100;                            // 0–<4 kt: ideal
}

// Secondary, informational factor only. Gust is not the regulatory
// criterion (see windProbability above) but real turbulence/handling still
// matters in practice, so a very high gust forecast applies a gentle
// downward adjustment rather than a hard cancellation.
function gustSoftRisk(gustKt: number) {
  if (!Number.isFinite(gustKt)) return 100;
  if (gustKt >= 25) return 60;
  if (gustKt >= 20) return 80;
  return 100;
}
...
function ceilingProbabilityFromHeight(ceilingFt: number) {
...
/**
 * LCL is only a cloud-base estimate. It must not be treated as an aviation
 * ceiling by itself. For the operational probability calculation, the LCL
 * height table is applied only when low-cloud coverage is at least ~5/8
 * (63%), which is used here as a BKN/OVC proxy.
 *
 * If low-cloud coverage is below 63%, the estimated LCL is displayed for
 * information but does not reduce the flight probability.
 */
function operationalCeilingProbability(
  estimatedLclFt: number,
  lowCloudPct: number
) {
  if (!Number.isFinite(estimatedLclFt)) return 100;
  if (!Number.isFinite(lowCloudPct)) return 100;
  if (lowCloudPct < 63) return 100;
  return ceilingProbabilityFromHeight(estimatedLclFt);
}
...
function aggregateDay(
  ...
  //
  // Gust strategy:
  // - 0–72 hours: use the short-range ICON gust only. Its hourly gust is the
  //   better operational match for the Göreme sunrise window.
  // - Beyond 72 hours: use the more conservative of ICON and ECMWF gusts.
  //
  // IMPORTANT: all gust samples below are still restricted to the exact
  // Göreme sunrise analysis window and the 10 km operational area.
  const iconGustKt = maxOf(
    operational
      .filter(r => String(r.model).startsWith('ICON'))
      .map(r => r.gust10Kt)
  );

  const ecmwfGustKt = maxOf(
    operational
      .filter(r => String(r.model).startsWith('ECMWF'))
      .map(r => r.gust10Kt)
  );

  const windowStartLocal = `${date}T${hm(sunriseMin - 30)}:00+03:00`;
  const leadHours = (Date.parse(windowStartLocal) - Date.now()) / 3600000;

  let maxGustKt: number;
  let gustStrategy: string;

  if (leadHours <= 72 && Number.isFinite(iconGustKt)) {
    maxGustKt = iconGustKt;
    gustStrategy = 'short-range-primary';
  } else {
    const availableLongRangeGusts = [iconGustKt, ecmwfGustKt].filter(Number.isFinite);
    maxGustKt = availableLongRangeGusts.length
      ? Math.max(...availableLongRangeGusts)
      : NaN;
    gustStrategy = 'long-range-conservative-blend';
  }

  // Fallback: if the preferred source is unexpectedly unavailable, use any
  // valid gust rather than fabricating a zero.
  if (!Number.isFinite(maxGustKt)) {
    maxGustKt = maxOf(operational.map(r => r.gust10Kt));
    gustStrategy = 'fallback-any-valid-gust';
  }
  ...
  // Storm/CB scan: 10 km operational area only (capped per pilot judgment).
  ...
  const windPct = windProbability(maxWindKt);
  const gustSoftPct = gustSoftRisk(maxGustKt);
  ...
  const ceilingCoverageRelevant =
    Number.isFinite(maxLowCloudPct) && maxLowCloudPct >= 63;

  const ceilingPct = operationalCeilingProbability(
    minCeilingFt,
    maxLowCloudPct
  );
  ...
  if (Number.isFinite(maxWindKt) && maxWindKt > 10) {
    vetoReasons.push(`10 km sustained ground wind > 10 kt per SHT-OPS-B (${round(maxWindKt)} kt)`);
  }
  ...
  // gustSoftPct is intentionally excluded here — it is a soft, secondary
  // factor and should not by itself trigger the multi-factor suppression.
  ...
  return {
    ...
    weather: {
      ...
      ceilingAppliedToProbability: ceilingCoverageRelevant,
      ceilingCoverageThresholdPct: 63,
    ...
    surface: {
      ...
      gustLeadHours: round(leadHours, 1),
      gustStrategy,
      shortRangeMaxGustKt: round(iconGustKt),
      secondaryMaxGustKt: round(ecmwfGustKt),
...
export const GET: APIRoute = async () => {
  try {
    ...
    return new Response(JSON.stringify({
      ...
      source: 'ECMWF + DWD-ICON (Open-Meteo)',
      methodology: {
        flightWindow: 'Göreme sunrise -30 min to +60 min, TRT (UTC+3)',
        operationalArea: 'Göreme center + 10 km operational ring',
        stormArea: 'Thunderstorm/CB proxy scan limited to the 10 km operational area',
        ...
        gustStrategy: '0–72 h: short-range hourly gust is primary; beyond 72 h: conservative multi-model gust blend. All gust samples are limited to the Göreme sunrise -30/+60 min window and the 10 km operational area. Gust is a secondary, soft factor — the binding criterion is sustained ground wind per SHT-OPS-B (10 kt for passenger flights).',
        combination: 'sustained wind × gust (soft) × precipitation × estimated ceiling × visibility, then hard veto rules',
        ceiling: 'LCL estimate only; not an observed METAR ceiling. It affects probability only when low-cloud coverage reaches the 63% BKN/OVC proxy threshold.'
      },
      days
    }), {
      status: 200,
